...
The post-fix manual closeout verified the counter, trigger, recipient, notification and shared alarm-event flow end to end. Known non-blocking carryover: when Resolve is rejected because the node is still unsafe, the backend correctly prevents resolution, but the Company Alarm UI shows no visible error message. Phase 13 must surface the backend conflict/validation response as a localized toast or inline error. The failed mutation must leave alarm status unchanged and reset loading state correctly.
