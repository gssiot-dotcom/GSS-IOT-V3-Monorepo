...
## Pre-Phase-14 refactor and UX completion wave (3rd step)

Status: `PRE_PHASE_14_REFACTOR_COMPLETE` as of 2026-07-22. Tasks 02 through 13 are complete. This is a staged pre-Phase-14 product refactor. It preserves `PHASE_13_COMPLETE` and does not start Phase 14.

Execution order is defined by `docs/prompts/3rd-step/00_MASTER_EXECUTION_PROMPT.md` and is tracked in `docs/prompts/3rd-step/EXECUTION_STATE.md`. Task 02 completed the baseline audit and created `docs/refactor/PRE_PHASE_14_REFACTOR_PLAN.md` plus `docs/quality/PRE_PHASE_14_REFACTOR_ACCEPTANCE_CHECKLIST.md`. Task 03 froze shared GSS tokens, exported reusable dashboard/action/realtime primitives, grouped permission-filtered navigation and added restrained shell/accessibility foundations. The current code audit confirmed the source-map gaps while preserving all existing RBAC/scope, assignment history, MQTT requestId/ACK, alarm/notification, and report behavior.

Tasks 04 and 05 are complete: authenticated Welcome/profile routes use safe session metadata, quick links are derived from permission-filtered navigation, the account menu is view-only plus sign-out, notification realtime state is truthful and permission-gated, and both dashboards now use bounded scope-safe operational summaries, KPI cards, charts, range selection and retained report jobs. Task 09 proved durable state was required and added only additive provisioning-mode/final-membership/replacement-history migrations; the existing GatewayCommand and assignment history protocol remains unchanged. No retention, production storage/deployment, rollback, or CI/CD hardening work is included.

Task 06 adds permission-guarded GSS role CRUD using the existing role/permission models, a redacted read-only system readiness page and server-derived Company contact settings. Task 07 adds guarded device edits, pristine-only hard deletion, server-derived history blockers, structured conflicts, audit preservation and compact accessible actions. Task 08 adds a shared canonical single/range/list parser, atomic auditable bulk node creation, duplicate/validation conflict responses and preview/count UX without changing MQTT assignment. Task 09 adds explicit APPEND/REPLACE provisioning membership, strict-ACK removal history, nonterminal concurrency protection and mode preview UX. Task 10 adds reusable Company monitoring TABLE/CARD and bounded detail-chart presentation with fault-filter separation. Task 11 adds global Admin monitoring aggregates, selector cascade, shared drilldown cards/detail and selected-room realtime. Task 12 adds the frozen responsive/accessibility conventions, representative desktop/mobile smoke coverage and the minimal contracts ESM boundary correction. Task 13 runs the full configured regression, records honest public browser/manual evidence and leaves authenticated visual review plus live ESP32 verification as explicit follow-ups. No unsafe runtime setting mutation or seed change is introduced. The task order is: 02 preflight; 03 design system/shell; 04 Welcome/profile/header/realtime; 05 dashboards; 06 Admin roles/settings; 07 device actions; 08 bulk nodes; 09 APPEND/REPLACE provisioning; 10 monitoring cards/detail charts; 11 Admin monitoring; 12 global responsive polish; 13 final regression and handoff.
