...
export class S3ReportStorageProvider implements ReportStorageProvider {
  readonly name = "s3";
  private readonly endpoint: URL;

  constructor(private readonly env: ApiEnv) {
    this.endpoint = new URL(
      env.REPORT_S3_ENDPOINT ?? `https://s3.${env.REPORT_S3_REGION}.amazonaws.com`,
    );
  }
  ...
  private async request(
    ...
    const authorization = signRequest(this.env, method, url, headers, payloadHash, dateStamp);
  ...
  private objectUrl(storageKey: string): URL {
    ...
    const forcePathStyle =
      this.env.REPORT_S3_FORCE_PATH_STYLE ?? Boolean(this.env.REPORT_S3_ENDPOINT);
    if (forcePathStyle) {
      url.pathname = `${prefix}/${encodeURIComponent(this.env.REPORT_S3_BUCKET!)}/${encodedKey}`;
    } else {
      url.hostname = `${this.env.REPORT_S3_BUCKET!}.${url.hostname}`;
...
function signRequest(
  ...
  const credentialScope = `${dateStamp}/${env.REPORT_S3_REGION}/s3/aws4_request`;
  const stringToSign = [
    "AWS4-HMAC-SHA256",
  ...
  const signingKey = hmac(
    hmac(
      hmac(hmac(`AWS4${env.REPORT_S3_SECRET_ACCESS_KEY!}`, dateStamp), env.REPORT_S3_REGION!),
      "s3",
    ),
    "aws4_request",
  );
  const signature = hmac(signingKey, stringToSign).toString("hex");
  return `AWS4-HMAC-SHA256 Credential=${env.REPORT_S3_ACCESS_KEY_ID!}/${credentialScope}, SignedHeaders=${signedHeaderNames.join(";")}, Signature=${signature}`;
}
